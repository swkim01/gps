...
gcj02.forward = forEachPoint(function(input, output, offset) {
  var lat = Math.max(Math.min(MAX_LATITUDE, input[offset + 1]), -MAX_LATITUDE);
  //var eval = Math.log(Math.tan(lat * RAD_PER_DEG) + 1 / Math.cos(lat * RAD_PER_DEG));
  var sin = Math.sin(lat * RAD_PER_DEG);

  output[offset] = (1 << 25) * (input[offset] + 180) / 360;
  //output[offset + 1] = (1 << 25) * eval / Math.PI / 4;
  output[offset + 1] = (1 << 25) * Math.log((1 + sin) / (1 - sin)) / Math.PI / 4;
});

gcj02.inverse = forEachPoint(function(input, output, offset) {
  var sign = (input[offset+1] < 0) ? -1 : 1;
  output[offset] = input[offset] * 360 / (1 << 25) - 180;
  //output[offset + 1] = Math.atan(sign * Math.sinh(Math.PI * 2 * input[offset + 1] / (1 << 25))) * 180 / Math.PI;
  output[offset + 1] = (2 * Math.atan(Math.exp(2 * input[offset + 1] * Math.PI / (1 << 25))) - (Math.PI / 2)) / RAD_PER_DEG;
});
